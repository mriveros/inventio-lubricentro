...
class SellData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (invoice_code,comment,ref_id,person_id,stock_to_id,iva,f_id,p_id,d_id,total,discount,cash,user_id,created_at) ";
		$sql .= "value (\"$this->invoice_code\",\"$this->comment\",$this->ref_id,$this->person_id,$this->stock_to_id,$this->iva,$this->f_id,$this->p_id,$this->d_id,$this->total,$this->discount,$this->cash,$this->user_id,$this->created_at)";
...
public function add_with_client(){	
	...
	public function update(){
		$sql = "update ".self::$tablename." set f_id=$this->f_id,person_id=$this->person_id,invoice_code=\"$this->invoice_code\",invoice_file=\"$this->invoice_file\",comment=\"$this->comment\", p_id=\"$this->p_id\"  where id=$this->id";
		Executor::doit($sql);
	}
