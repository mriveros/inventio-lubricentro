...
class SpendData {
	...
	public function add(){
		$sql = "insert into spend (name,kind,price,created_at) ";
		$sql .= "value (\"$this->name\",$this->kind, $this->price,$this->created_at)";
		Executor::doit($sql);
	}
...
// partiendo de que ya tenemos creado un objecto CategoryData previamente utilizamos el contexto
	...
	public static function getAllUnBoxed(){
		$sql = "select * from ".self::$tablename." where box_id is NULL and (kind=1 or kind=2) order by created_at desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new SpendData());
	}

	public static function getDepUnBoxed(){
		$sql = "select * from ".self::$tablename." where box_id is NULL and (kind=3) order by created_at desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new SpendData());
	}
	...
	public static function getAllByCategoryId($category_id){
		...
		public static function getGroupByDateOp($start,$end){
 		$sql = "select *,sum(price) as t from ".self::$tablename." where date(created_at) >= \"$start\" and date(created_at) <= \"$end\" and (kind=1 or kind=2)";
		$query = Executor::doit($sql);
		return Model::many($query[0],new SpendData());
	}
