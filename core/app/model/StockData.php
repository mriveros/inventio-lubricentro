...
class StockData {
	...
	public function add(){
		$sql = "insert into stock (code,field1,field2,name,address,phone,email) ";
		$sql .= "value (\"$this->code\",\"$this->field1\",\"$this->field2\",\"$this->name\",\"$this->address\",\"$this->phone\",\"$this->email\")";
		Executor::doit($sql);
	}
...
// partiendo de que ya tenemos creado un objecto StockData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set code=\"$this->code\",field1=\"$this->field1\",field2=\"$this->field2\",name=\"$this->name\",address=\"$this->address\",phone=\"$this->phone\",email=\"$this->email\" where id=$this->id";
		Executor::doit($sql);
	}
