...
class PaymentData {
	...
	public function add_payment(){
		$sql = "insert into ".self::$tablename." (sell_id,person_id,val,payment_type_id,created_at) ";
		$sql .= "value ($this->sell_id,\"$this->person_id\",$this->val,2,NOW())";
		Executor::doit($sql);
	}
...
public static function sumAllByDate($start,$end){
		$sql = "select abs(sum(val)) as t from ".self::$tablename." where (date(created_at)>=\"$start\" and date(created_at)<=\"$end\") and payment_type_id=2 order by created_at desc";
		$query = Executor::doit($sql);
		return Model::one($query[0],new PaymentData());	
	}
	...
	public static function getAllByClientId($id){
		$sql = "select * from ".self::$tablename." where person_id=$id order by created_at asc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new PaymentData());	
	}
	...
	public static function sumBySellId($id){
		...
		public static function getGroupByDateOp($start,$end){
 		$sql = "select *,sum(abs(val)) as t from ".self::$tablename." where payment_type_id=2 and date(created_at) >= \"$start\" and date(created_at) <= \"$end\"";
		$query = Executor::doit($sql);
		return Model::many($query[0],new PaymentData());
	}
