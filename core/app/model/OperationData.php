...
public static function get10Popular($stock, $start,$end){
 $sql = "select *,sum(q) as total from ".self::$tablename." where (date(created_at) >= \"$start\" and date(created_at) <= \"$end\") and operation_type_id=2 and is_draft=0 and status=1 group by product_id order by total desc limit 10";
		$query = Executor::doit($sql);
		return Model::many($query[0],new OperationData());
	}
