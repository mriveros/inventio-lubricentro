...
class ProductData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (image,kind,code,brand_id,width,height,weight,barcode,name,description,price_in,price_out,price_out2, price_out3, user_id,presentation,unit,category_id,inventary_min,expire_at,created_at) ";
		$sql .= "value (\"$this->image\",\"$this->kind\",\"$this->code\",$this->brand_id,\"$this->width\",\"$this->height\",\"$this->weight\",\"$this->barcode\",\"$this->name\",\"$this->description\",\"$this->price_in\",\"$this->price_out\",\"$this->price_out2\",\"$this->price_out3\", $this->user_id,\"$this->presentation\",\"$this->unit\",$this->category_id,$this->inventary_min,\"$this->expire_at\",NOW())";
...
// partiendo de que ya tenemos creado un objecto ProductData previamente utilizamos el contexto
	...
	public static function getByBarcode($id){
		$sql = "select * from ".self::$tablename." where barcode=\"$id\"";
		$query = Executor::doit($sql);
		return Model::one($query[0],new ProductData());
