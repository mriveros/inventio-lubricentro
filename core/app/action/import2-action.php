...
//                echo "?";
          ...
          // lat, lng = 10 y 11
    			...
    			echo $sql = "insert into product (name,olt,nopoint,barcode,ncams,location, tiposite, city, municipe, nic, nocircuit, tipocircuit, respaldo, tipobatery, nbatery, created_at) value (\"$data[1]\",\"$data[2]\",\"$data[3]\",\"".utf8_encode($data[4])."\",\"".utf8_encode($data[5])."\",\"".utf8_encode($data[6])."\",\"".utf8_encode($data[7])."\",\"".utf8_encode($data[8])."\",\"".utf8_encode($data[9])."\",\"".utf8_encode($data[14])."\",\"".utf8_encode($data[15])."\",\"$data[16]\",\"$data[17]\",\"$data[18]\",\"$data[19]\",NOW());<br>";
