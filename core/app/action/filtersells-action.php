...
$whereparams = array();
$whereparams[] = " (operation_type_id=2 and p_id=1 and d_id=1 and is_draft=0) ";
...
if(isset($_GET["start_at"]) && isset($_GET["finish_at"]) && $_GET["start_at"]!="" && $_GET["finish_at"]!=""){
	...
	$whereparams[] = " ( DATE_FORMAT(created_at, '%Y/%m/%d') >= (DATE_FORMAT('$_GET[start_at]','%Y/%m/%d')) and DATE_FORMAT(created_at, '%Y/%m/%d') <=  (DATE_FORMAT('$_GET[finish_at]', '%Y/%m/%d')) ) ";
